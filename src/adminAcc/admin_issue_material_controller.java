...
public class admin_issue_material_controller implements Initializable {
	...
	private void issueMaterial(ActionEvent event) {
		...
		try {
			psCheckRecord = dbConnect.prepareStatement("SELECT * FROM records WHERE ref_num = '"
					+ admin_currReserved_controller.records.getRef_num() + "';");
			rsRecord = psCheckRecord.executeQuery();
			if (rsRecord.next()) {
				try {
					psUpdateRecord = dbConnect.prepareStatement("UPDATE records SET issued_date = '"
							+ LocalDate.now().toString() + "', due_date = '" + LocalDate.now().plusDays(30)
							+ "' WHERE ref_num = '" + admin_currReserved_controller.records.getRef_num() + "';");
	...
	private void removeReserve(ActionEvent event) {
		...
		try {
			...
			if (rsRecord.next()) {
				try {
					psUpdateCopies = dbConnect.prepareStatement(
							"UPDATE material SET copies_avail = copies_avail + 1 WHERE material_id = '"
									+ admin_currReserved_controller.records.getMaterial_id() + "'");
					psRemoveReserve = dbConnect.prepareStatement("DELETE FROM records WHERE ref_num = '"
							+ admin_currReserved_controller.records.getRef_num() + "';");
