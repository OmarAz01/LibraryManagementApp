...
public class admin_userReport_controller implements Initializable {
	...
	private void initializeUserHistoryTable() {
		...
		try {
			...
			psGetRecord = dbConnect.prepareStatement("SELECT * FROM records WHERE user_id = '" + admin_report_controller.account.getUser_id() + "';");
